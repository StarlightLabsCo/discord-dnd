...
import { RollDiceRequest } from "starlight-api-types/websocket/requests.js";
...
export class D20Dice implements SceneSubject {
    ...
    onClick(): void {
        ...
        if (!ws) {
            console.error(
                "No websocket connection found to send roll dice request."
            );
            return;
        }
